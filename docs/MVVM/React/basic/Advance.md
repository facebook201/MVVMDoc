...
带有 render prop的组件 带有一个返回一个React元素的函数并调用该函数而不是实现自己的渲染逻辑。



* 组件是 react 最基础的代码重用单元，当有些地方可以复用 我们会抽象出来
* 传统的父组件中的子组件通过 props.children 渲染出来，比较单一 传什么就渲染什么到子组件
* render prop 可以动态决定什么需要渲染 而且可以传参数
...
* Hook是一个特殊的函数， 可以勾入React的特性

* Hook在函数组件里面使用 不能在类组件里面使用
